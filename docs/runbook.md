...
| `AGORA_RETENTION_DAYS`              | `90`                                                   | Days post-terminal-state after which a saga's borrower fields are scrubbed. ALA model policy + state library-record statutes drive the 90-day default. Set lower (e.g. 30) for stricter jurisdictions. |
| `AGORA_RETENTION_SCAN_INTERVAL_SECS`| `3600`                                                 | Sleep between retention scanner sweeps. Hourly is enough — eligibility advances day-by-day. |
| `AGORA_PII_SCRUB_SALT`              | `""`                                                   | HMAC salt for the scrub fingerprint. Production MUST rotate a 32-byte secret (`python -c 'import secrets; print(secrets.token_hex(32))'`). Empty value fails the scrubber closed (`RetentionConfigError`) so the DSAR endpoints surface a 503 rather than silently anonymising with a zero salt. |
