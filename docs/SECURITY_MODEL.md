...
| `/sagas/{id}/discover`                                                                                          | Outbound CrossRef + SRU calls per request — burns third-party rate budget and may leak request metadata. |
...
| `POST /sagas/{id}/discover`                | `staff:<u>@<lib>`   | JSON API         | Same auth + scope. Refuses on terminal saga. Candidate / diagnostics list capped to 50 (audit #19).                                                                                                                |
...
| Saga ledger append                         | All                 | Internal         | `UNIQUE(idempotency_key)` at DB layer is the load-bearing replay-safety primitive (mirrored in `idempotency_key` collision validation — audit #22). Terminal-state guard refuses non-OBSERVATION events.           |
...
- Application-layer state-machine wrapper (`Coordinator.run_forward` requires a committed gate; direct ledger writes forbidden by convention)
...
| Test suite (full)                                  | 541 tests + 11 postgres-only / sandbox-only skipped                                     | green                                                                                                           |
