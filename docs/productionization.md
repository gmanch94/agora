...
| G-02   | Authorisation         | All staff users see everything. No role separation.                    | RBAC: viewer / approver / admin / cross-consortium-relay.             | **P0**   | §4.2          |
...
| G-07   | Patron PII            | `PatronProfile.patron_id` + barcode stored on saga events indefinitely. No retention policy. | Documented retention window (ALA recommends destroy-on-completion; some states mandate). Purge job + tested DSAR (data-subject access request) flow. | **P0**   | §4.3          |
