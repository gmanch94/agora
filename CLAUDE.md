...
# Verify
.venv/Scripts/python.exe -m pytest tests/ -q              # 665 tests (+6 postgres-only).venv/Scripts/python.exe -m ruff check src tests          # lint
...
│   ├── adr/  (20 docs)          # Architecture decisions
...
├── tests/                       # 665 tests (unit + property + e2e)├── docker-compose.yml           # Postgres-only sandbox
