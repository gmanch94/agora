...
# Verify
.venv/Scripts/python.exe -m pytest tests/ -q              # 460 tests (+6 postgres-only).venv/Scripts/python.exe -m ruff check src tests          # lint
...
├── tests/                       # 460 tests (unit + property + e2e)├── docker-compose.yml           # Postgres-only sandbox
