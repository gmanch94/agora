...
(`agora.demos.happy_path`). **451 tests** green (+6 postgres-only in CI).Saga + outbox + APPROVING-via-outbox (ADR-0012), multi-worker outbox
...
├── tests/               # 451 unit + property + e2e (+6 postgres-only)├── .github/workflows/   # audit.yml, postgres-tests.yml, triple-gate.yml,
